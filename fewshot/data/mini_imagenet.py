...
class MiniImageNetDataset(RefinementMetaDataset):
    ...
    def __init__(self,
        ...
        super(MiniImageNetDataset,
              self).__init__(args, split, nway, nshot, num_unlabel,
                             num_distractor,
                             num_test, label_ratio, mode_ratio, train_modes,
                             cat_way, seed)
        ...
        with open(csv_filename) as csv_file:
            ...
            for (image_filename, class_name) in csv_reader:
                if 'label' not in class_name:
                    img = imread(self.images_path + image_filename)
                    img = img.reshape((self.n_input, self.n_input, 3))
