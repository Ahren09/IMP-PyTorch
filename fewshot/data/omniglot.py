...
class OmniglotDataset(RefinementMetaDataset):
    ...
    def __init__(self,
        ...
        if args.disable_distractor:
            num_distractor = 0
        super(OmniglotDataset, self).__init__(args, split, nway, nshot, num_unlabel, num_distractor, num_test,
                                              label_ratio, mode_ratio, train_modes, cat_way, seed)

    def get_images(self,
            inds):
        return self._images[inds]

    def get_converted_images(self,
            inds,
            transform):
        if transform:
    ...
    def process_category_labels(self,
            labels):
        i = 0
        mydict = {}
    ...
    def read_label_split(self):
        ...
        if os.path.exists(cache_path_labelsplit):
            ...
            if self._split in ['train', 'trainval']:
                print('Use {}% image for labeled split.'.format(int(self._label_ratio * 100)))
    ...
    def read_dataset(self):
        ...
        if not self.read_cache():
            ...
            if split_def == 'lake':
                self._images, self._labels, self._label_str, self._category_labels = read_lake_split(folder,
                    aug_90=self._aug_90)
            elif split_def == 'vinyals':
                split_file = get_vinyals_split_file(self._split)
                # self._images, self._labels, self._label_str, self._category_labels = read_vinyals_split(folder, split_file, aug_90=self._aug_90, merged=self._multimodal)
                self._images, self._labels, self._label_str, self._category_labels = read_vinyals_split(folder, split_file, aug_90=self._aug_90, merged=False)
            self.read_label_split()
    ...
    def get_label_split_path(self):
        ...
        if self._split in ['train', 'trainval']:
            cache_path = os.path.join(self._folder,
                self._split + split_def_str + aug_str + '_labelsplit' + label_ratio_str + seed_id_str + '.txt')
        elif self._split in ['val', 'test']:
            cache_path = os.path.join(self._folder, self._split + split_def_str + aug_str + '_labelsplit' + '.txt')
        return cache_path
    ...
    def get_cache_path(self):
        ...
        aug_str = '_aug90' if self._aug_90 else ''
        split_def_str = '_' + self._split_def
        cache_path = os.path.join(self._folder, self._split + split_def_str + aug_str + '.pkl')
