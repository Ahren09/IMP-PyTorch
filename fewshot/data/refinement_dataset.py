...
class RefinementMetaDataset(object):
    ...
    def __init__(self, args, split, nway=5, nshot=1, num_unlabel=2,
                 num_distractor=0, num_test=10,
                 label_ratio=1., mode_ratio=1., train_modes=True, cat_way=5.,
                 seed=0):
    ...
    def mode_split(self):
        ...
        for sup in range(0, len(self.coarse_labels)):
            mode_split.extend(list(np.random.choice(self.coarse_labels[sup],
                                                    max(1,
                                                        int(self._mode_ratio * len(
                                                            self.coarse_labels[
                                                                sup]))),
                                                    replace=False)))
    ...
    def read_mode_split(self):
        ...
        if os.path.exists(cache_path_modesplit):
            self._class_train_set = np.loadtxt(cache_path_modesplit,
                                               dtype=np.int64)
        else:
    ...
    def filter_classes(self, class_seq):
        idxs = list(
            filter(lambda _id: self._label_general[_id] in self.okay_classes[
                self._split], class_seq))
        return idxs
    ...
    def next_episode(self, within_category=False):
        ...
        if within_category and self._cat_way != -1:
            ...
            for cat_idx in cat_idxs:
                current_inds = \
                np.where(np.array(self._category_labels) == cat_idx)[0]
                filtered_inds = list(
                    filter(lambda _id: _id in self.class_seq, current_inds))
                self._rnd.shuffle(filtered_inds)
                allowable_inds = np.concatenate((allowable_inds, filtered_inds[
                                                                 0:min(
                                                                     self._nway,
                                                                     len(filtered_inds))]))
        ...
        if hasattr(self, "_category_labels"):
            train_labels_str = np.hstack((np.array(train_labels_str)[:, None],
                                          np.array(self.episodic_labels(
                                              train_labels_str))[:, None]))
            test_labels_str = np.hstack((np.array(test_labels_str)[:, None],
                                         np.array(self.episodic_labels(
                                             test_labels_str))[:, None]))
        else:
