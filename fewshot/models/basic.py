...
class Protonet(nn.Module):
    ...
    def _run_forward(self, cnn_input):
        ...
        encoded = self.encoder.forward(cnn_input.view(n_class * n_support, *cnn_input.size()[2:])) # (5, 1, 28, 28)
    ...
    def  _compute_protos(self, h, probs):
        ...
        h = torch.unsqueeze(h, 2)       # [B, N, 1, D] = (1, 5, 1, 64) or (1, 30, 1, 64)
        probs = torch.unsqueeze(probs, 3)       # [B, N, nClusters, 1] = (1, 5, 5, 1)
        prob_sum = torch.sum(probs, 1)  # [B, nClusters, 1] = (1, 5, 1)
        ...
        protos = h*probs    # [B, N, nClusters, D] = (1, 5, 5, 64)
