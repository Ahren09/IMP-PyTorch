...
def assign_cluster_radii_limited(cluster_centers, data, radii, target_labels):
    ...
    # Compute the weight (logits) of each example wrt each cluster center
    logits = compute_logits_radii(cluster_centers, data, radii) # [B, N, K]
...
def compute_logits_radii(cluster_centers, data, radii, prior_weight=1.):
    ...
    """
    cluster_centers = torch.unsqueeze(cluster_centers, 1)   # [B, 1, K, D] = (1, 1, 5, 64)
    data = torch.unsqueeze(data, 2)  # [B, N, 1, D] = (1, 50, 1, 64)
    dim = data.size()[-1]
    radii = torch.unsqueeze(radii, 1)  # [B, 1, K] = (1, 1, 5) or (1, 1, 7)
    neg_dist = -torch.sum((data - cluster_centers)**2, dim=3)   # [B, N, K] = (1, 50, 5) or (1, 10, 7)
